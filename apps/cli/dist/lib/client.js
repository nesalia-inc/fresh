...
export async function requestDeviceCode(clientId) {
    ...
    try {
        ...
        if (error.message.includes("fetch failed") || error.message.includes("ECONNREFUSED") || error.message.includes("ENOTFOUND")) {
            throw createCLIError(`Cannot connect to ${API_BASE}. Is the Fresh server running?\n` +
                `Hint: Set FRESH_API_URL environment variable to your server URL.\n` +
                `Example: FRESH_API_URL=http://localhost:3000 fresh auth login`, { code: "NETWORK_ERROR", endpoint: "/device/code" });
        }
...
export async function pollForToken(deviceCode, clientId, initialInterval = 5000) {
    ...
    while (true) {
        ...
        try {
            ...
            if (error.message.includes("fetch failed") || error.message.includes("ECONNREFUSED") || error.message.includes("ENOTFOUND")) {
                throw createCLIError(`Cannot connect to ${API_BASE}. Is the Fresh server running?\n` +
                    `Hint: Set FRESH_API_URL environment variable to your server URL.`, { code: "NETWORK_ERROR", endpoint: "/device/token" });
            }
        ...
        if (data.error) {
            switch (data.error) {
                ...
                case "slow_down":
                    ...
                    continue;
                case "expired_token":
                    throw createCLIError("Code expired. Run 'fresh auth login' to try again.", { code: "EXPIRED_TOKEN", endpoint: "/device/token" });
                case "access_denied":
                    throw createCLIError("Access denied by user. Run 'fresh auth login' to try again.", { code: "ACCESS_DENIED", endpoint: "/device/token" });
                case "authorization_declined":
                    throw createCLIError("Authorization was declined. Run 'fresh auth login' to try again.", { code: "AUTHORIZATION_DECLINED", endpoint: "/device/token" });
                default:
                    throw createCLIError(data.errorDescription || `OAuth error: ${data.error}`, { code: data.error, statusCode: response.status, endpoint: "/device/token" });
            }
        }
        return data;
    }
